...
class User extends Connect {
      ...
      public static function register($id , $email , $password , $name , $username) {
        $user_id = $id;
        $pdo = self::connect();
        $pdo->beginTransaction();      
        $stmt = $pdo->prepare("INSERT INTO `users` (`id` , `email` , `password` , `name` , `username`) Values (:id , :email , :password , :name , :username)");
