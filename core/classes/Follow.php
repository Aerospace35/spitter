...
class Follow extends User {
	...
	public static function whoToFollowAll($user_id){
		$stmt = self::connect()->prepare("SELECT * FROM `users`
         WHERE `id` != :user_id AND `id` NOT IN 
         (SELECT `following_id` FROM `follow` WHERE `follower_id` = :user_id) ORDER BY rand() LIMIT 8");
		// $stmt->execute(array("user_id" => $user_id));
        $stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
        $stmt->execute(); 
		return $stmt->fetchAll(PDO::FETCH_OBJ);	
	}
	public static function whoToFollowAll2($user_id){
		$stmt = self::connect()->prepare("SELECT * FROM `users`
         WHERE `id` != :user_id AND `id` NOT IN 
         (SELECT `following_id` FROM `follow` WHERE `follower_id` = :user_id) ORDER BY rand()");
		// $stmt->execute(array("user_id" => $user_id));
        $stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
        $stmt->execute(); 
		return $stmt->fetchAll(PDO::FETCH_OBJ);	
